...
alias circuit-envoy="FORTIO_POD=$(kubectl get pod | grep fortio | awk '{ print $1 }'); kubectl exec -it $FORTIO_POD -c istio-proxy -- sh -c 'curl localhost:15000/stats' | grep shipping | grep pending_overflow"
